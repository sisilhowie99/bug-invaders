...
function create() {
    ...
    //create bullets instances;
    const pellets = this.physics.add.group();

    function genPellet() {
        //Phaser.Utils.Array.GetRandom takes an array and selects random element from that array
        //.getChildren() returns an array belonging to a group object
...
function update() {
    if (gameState.active) {
        ...
        if (numOfTotalEnemies() === 0) {
            ...
            gameState.active = false;
            this.physics.pause();
            this.add.text(180, 290, 'You won!', { fontSize: '15px', fill: '#000000' });
        } else {
